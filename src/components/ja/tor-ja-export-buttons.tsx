...
export function TorJaExportButtons({
  torDocumentId,
  year,
}: {
  ...
  async function download(format: "pdf" | "docx") {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/tor/${torDocumentId}/export?format=${format}`);
      if (!response.ok) {
      ...
      anchor.href = url;
      anchor.download = `JA-TOR-${year}.${format === "pdf" ? "pdf" : "docx"}`;
      anchor.click();
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <div className="flex flex-wrap justify-end gap-2">
        ...
        >
          {busy === "pdf" ? "กำลังสร้าง…" : "PDF ทั้งฉบับ"}
        </button>
        <button
          type="button"
          disabled={busy !== null}
          ...
          className="rounded-lg border border-stone-300 px-3 py-1.5 text-xs font-medium text-stone-700 hover:bg-stone-50 disabled:opacity-50"
        >
          {busy === "docx" ? "กำลังสร้าง…" : "Word ทั้งฉบับ"}
        </button>
      </div>
