...
export type IdentityProfile = { subject: string; email: string; employeeId: string; firstName: string; lastName: string; title?: string; position?: string };
...
export async function createAuthorizationUrl(state: string, nonce: string) {
  if (!env.CMU_CLIENT_ID || !env.CMU_REDIRECT_URI) throw new Error("CMU OIDC is not configured");
  const config = await discovery();
  const url = new URL(config.authorization_endpoint);
  url.search = new URLSearchParams({ client_id: env.CMU_CLIENT_ID, redirect_uri: env.CMU_REDIRECT_URI, response_type: "code", scope: "openid profile email", state, nonce }).toString();
  return url;
}
...
export async function exchangeCode(code: string, nonce: string): Promise<IdentityProfile> {
  ...
  const { payload } = await jwtVerify(tokens.id_token, createRemoteJWKSet(new URL(config.jwks_uri)), { issuer: config.issuer, audience: env.CMU_CLIENT_ID });
  ...
  return { subject: payload.sub, email: String(payload.email).toLowerCase(), employeeId: String(payload.employee_id ?? payload.sub), firstName: String(payload.given_name ?? ""), lastName: String(payload.family_name ?? "") };
}
